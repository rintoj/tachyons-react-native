...
function addColor(name: string, color: string) {
  const style: any = {}
  style[`${name}`] = { color }
...
export function setTheme(theme: string) {
  if (Object.keys(themes).indexOf(theme) < 0) { throw new Error('Invalid theme name') }
  selectedTheme = theme
}
...
export function withTheme(TargetComponent: React.ComponentClass<any>): React.ComponentClass<any> {
  ...
  return class ThemeableComponent extends React.Component<any, { theme?: string }> {
    ...
    render() {
      return <TargetComponent {...this.props} theme={this.state.theme} setTheme={(theme: string) => this.setTheme(theme)} />
    }
  }
